...
function World:__init()
    ...
    local result = postgresClient.QueryAwait([[SELECT key FROM data_storage WHERE key = 'world']])
...
function World:QuicksaveToDrive()
    threadHandler.Async(function() self:SaveToDrive() end)
end

function World:LoadFromDrive()
    local result = postgresClient.QueryAwait([[SELECT data FROM data_storage WHERE key = 'world';]])
    if result.error then
